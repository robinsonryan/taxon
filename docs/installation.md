...
**`uuid7` mode needs PostgreSQL 18 or later.** The keys are generated by the
database, through the `uuidv7()` column default the migrations put on `tags.id`
and `taggables.id` — Taxon does not generate them in PHP. `uuidv7()` is a
PostgreSQL 18 built-in, so on any earlier server, or on MySQL or SQLite, the
migrations skip the default and inserts have nothing to supply a key.

Upgrading an install that predates 0.5.0? Its columns will not have that default
yet. See the 0.5.0 entry in [the changelog](../CHANGELOG.md) — there is a
migration to run, and skipping it breaks the next insert.
